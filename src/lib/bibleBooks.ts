export const BOOK_CHAPTERS: Record<string, number> = {
  gn: 50, ex: 40, lv: 27, nm: 36, dt: 34, js: 24, jz: 21, rt: 4,
  '1sm': 31, '2sm': 24, '1rs': 22, '2rs': 25, '1cr': 29, '2cr': 36,
  ed: 10, ne: 13, et: 10, job: 42, sl: 150, pv: 31, ec: 12, ct: 8,
  is: 66, jr: 52, lm: 5, ez: 48, dn: 12, os: 14, jl: 3, am: 9,
  ob: 1, jn: 4, mq: 7, na: 3, hc: 3, sf: 3, ag: 2, zc: 14, ml: 4,
  mt: 28, mc: 16, lc: 24, jo: 21, atos: 28, rm: 16, '1co': 16,
  '2co': 13, gl: 6, ef: 6, fp: 4, cl: 4, '1ts': 5, '2ts': 3,
  '1tm': 6, '2tm': 4, tt: 3, fm: 1, hb: 13, tg: 5, '1pe': 5,
  '2pe': 3, '1jo': 5, '2jo': 1, '3jo': 1, jd: 1, ap: 22,
};

export function getChapterCount(abbrev: string): number {
  return BOOK_CHAPTERS[abbrev] ?? 0;
}

export function getRandomChapter(abbrev: string): number {
  const count = getChapterCount(abbrev);
  if (count === 0) return 1;
  return Math.floor(Math.random() * count) + 1;
}

export interface VerseRef {
  bookName: string;
  abbrev: string;
  chapter: number;
  verse: number;
  text: string;
  reference: string;
}
